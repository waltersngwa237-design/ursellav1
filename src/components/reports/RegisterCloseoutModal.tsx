...
export const RegisterCloseoutModal: React.FC<RegisterCloseoutModalProps> = ({
  ...
  const handleCloseShift = async () => {
    ...
    if (Math.abs(discrepancy) > 0.01 && !isPinVerified) {
      if (!verifyManagerPin(managerPin)) {
        setPinError('Invalid manager PIN. Enter authorized manager PIN to approve discrepancy.');
        return;
      }
    }
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Register Closeout & Z-Report (${shift?.z_report_number || 'EOD'})`}
      maxWidth="lg"
    >
      <div className="space-y-5">
        {loading ? (
          <div className="py-12 text-center text-sm text-zinc-500">Loading register audit details...</div>
        ) : !shift ? (
          <div className="py-12 text-center text-sm text-rose-500">No shift record found.</div>
        ) : (
          <>
            {/* Shift Banner */}
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-mono font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">
                    Shift #{shift.shift_number} &bull; {shift.z_report_number}
                  </span>
                  <Badge variant={shift.status === 'open' ? 'amber' : 'emerald'}>
                    {shift.status === 'open' ? 'Active Shift (Open)' : 'Sealed & Closed (Z-Report)'}
                  </Badge>
                </div>
                <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
                  Opened: {new Date(shift.opened_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} by {shift.opened_by}
                </p>
              </div>

              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={handleKickDrawer} title="Test Cash Drawer Trigger">
                  Kick Drawer
                </Button>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
              <div className="p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
                ...
                <span className="text-[10px] text-zinc-400">{shift.sales_count} receipts</span>
              </div>
              ...
              <div className="p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
                ...
                <span className="text-[10px] text-zinc-400">Electronic Tender</span>
              </div>
              ...
              <div className="p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
                ...
                <span className="text-[10px] text-zinc-400">Physical tender</span>
              </div>
              ...
              <div className="p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
                ...
                <span className="text-[10px] text-zinc-400">Drawer expenses</span>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/40 space-y-3">
              ...
              <div className="space-y-1.5 text-xs">
                ...
                <div className="pt-2 border-t border-zinc-200 dark:border-zinc-700 flex justify-between font-bold text-sm">
                  <span className="text-zinc-900 dark:text-zinc-100">Expected Physical Cash in Drawer:</span>
              ...
              <div className="p-3.5 rounded-xl border border-amber-300 dark:border-amber-700/50 bg-amber-500/10 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-amber-700 dark:text-amber-300">
                    {showMovementForm === 'in' ? 'Record Cash In (Add to Drawer)' : 'Record Cash Out (Petty Cash Payout)'}
                  </span>
                  <button onClick={() => setShowMovementForm(null)} className="text-xs text-zinc-500 hover:text-zinc-700">
                    Cancel
                  </button>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  ...
                  <Input
                    ...
                    placeholder="e.g. Change replenishment, Store supplies"
                  />
                </div>
                <Button size="sm" onClick={handleRecordMovement} disabled={isSubmittingMovement}>
                  {isSubmittingMovement ? 'Recording...' : 'Confirm Cash Movement'}
                </Button>
              </div>
            )}
            ...
            {shift.status === 'open' ? (
              <div className="space-y-3 p-4 rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900">
                <div className="flex items-center justify-between">
                  ...
                  >
                    ...
                    {showDenominations ? 'Hide Denominations' : 'Denominations Calculator'}
                  </button>
                </div>
                ...
                {showDenominations && (
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 p-3 bg-zinc-50 dark:bg-zinc-800/40 rounded-lg text-xs">
                    {Object.keys(denominations).map((denom) => (
                      <div key={denom}>
                        <span className="text-[11px] text-zinc-500">{denom} note/coin</span>
                        <input
                          type="number"
                          min="0"
                  ...
                  <div className="shrink-0">
                    {Math.abs(discrepancy) < 0.01 ? (
                      ...
                      <Badge variant="blue" className="px-3 py-2 text-xs">
                        +{currencyConfig.format(discrepancy)} Over
                      </Badge>
                    ) : (
                      <Badge variant="rose" className="px-3 py-2 text-xs flex items-center gap-1">
                        <AlertTriangle className="w-4 h-4" />
                        -{currencyConfig.format(Math.abs(discrepancy))} Short
                      </Badge>
                    )}
                  </div>
                </div>
                ...
                <Input
                  label="Closeout Notes / Variance Explanation"
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder="Optional details regarding register closeout"
                />
                ...
                {Math.abs(discrepancy) > 0.01 && !isPinVerified && (
                  <div className="p-3 rounded-lg border border-amber-300 dark:border-amber-700/50 bg-amber-50 dark:bg-amber-950/30 space-y-2">
                    <div className="flex items-center gap-2 text-xs font-semibold text-amber-800 dark:text-amber-300">
                      ...
                      Manager Sign-off Required for Variance
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      <Input
                      ...
                      <Input
                        type="password"
                        placeholder="Manager PIN (default: 8888)"
                        value={managerPin}
                        onChange={(e) => {
              ...
              <div className="p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-zinc-500">Physical Cash Counted at Closeout:</span>
                  <span className="font-mono font-bold text-sm">
                    {currencyConfig.format(shift.actual_cash_counted)}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-zinc-500">Register Variance:</span>
                  <span className="font-mono font-bold text-sm">
                    {shift.discrepancy === 0
                      ? 'Balanced (0.00)'
                      : shift.discrepancy > 0
                      ? `+${currencyConfig.format(shift.discrepancy)} Over`
                      : `-${currencyConfig.format(Math.abs(shift.discrepancy))} Short`}
                  </span>
                </div>
                {shift.notes && (
