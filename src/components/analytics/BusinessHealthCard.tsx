...
export const BusinessHealthCard: React.FC<BusinessHealthCardProps> = ({ indicator }) => {
  ...
  return (
    <div className="p-5 rounded-2xl bg-white dark:bg-zinc-900/90 border border-slate-200/90 dark:border-zinc-800/90 shadow-xs">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-zinc-400">Deterministic operational health index</p>
          </div>
        </div>
      ...
      {/* Main Score Bar */}
      <div className="mb-5">
        <div className="flex justify-between text-xs text-slate-500 dark:text-zinc-400 mb-1.5 font-medium">
      ...
      <div className="space-y-3 pt-3 border-t border-slate-100 dark:border-zinc-800/80">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {drivers.map((d, i) => {
            let statusBadge = 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/20';
            if (d.status === 'warning') {
              statusBadge = 'bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 border-rose-200 dark:border-rose-500/20';
            } else if (d.status === 'moderate') {
              statusBadge = 'bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-500/20';
            }

            return (
              <div
                key={i}
                className="p-3 rounded-xl bg-slate-50/80 dark:bg-zinc-950/60 border border-slate-200/80 dark:border-zinc-800/70 flex flex-col justify-between"
              >
                <div className="flex items-center justify-between gap-2 mb-1.5">
                  <span className="text-xs font-semibold text-slate-800 dark:text-zinc-200">{d.name}</span>
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${statusBadge}`}>
                    {d.score}/100
                  </span>
                </div>
                <p className="text-xs text-slate-500 dark:text-zinc-400">{d.detail}</p>
              </div>
            );
          })}
        </div>
      ...
      <div className="mt-4 pt-3 border-t border-slate-100 dark:border-zinc-800/60">
        <button
          ...
          className="flex items-center justify-between w-full text-xs text-slate-500 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-zinc-200 cursor-pointer"
        >
        ...
        {showMethodology && (
          <div className="mt-2.5 p-3 rounded-xl bg-slate-50 dark:bg-zinc-950/80 border border-slate-200 dark:border-zinc-800/80 text-xs text-slate-600 dark:text-zinc-400 space-y-1.5">
            <p className="font-medium text-slate-800 dark:text-zinc-300">{methodology}</p>
