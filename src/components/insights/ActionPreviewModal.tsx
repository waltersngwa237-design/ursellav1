...
export const ActionPreviewModal: React.FC<ActionPreviewModalProps> = ({
  ...
  const [adjustmentQty, setAdjustmentQty] = useState<number>(
    insight.action_payload?.quantity ??
    insight.action_payload?.suggestedQuantity ??
    insight.action_payload?.adjustmentQuantity ??
    insight.action_payload?.targetQuantity ??
    10
  );
  ...
  const handleExecute = async () => {
    ...
    if (actionType === 'send_customer_message') {
      ...
      payload.suggestedQuantity = Number(adjustmentQty);
          ...
          {(actionType === 'create_restock_task' || actionType === 'create_reminder') && (
            <div className="space-y-3">
              ...
              {actionType === 'create_restock_task' && (
                <div className="space-y-3">
                  {(insight.action_payload?.productName || insight.action_payload?.name) && (
                    <div className="p-3 rounded-xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-xs">
                      ...
                      {insight.action_payload?.currentStock !== undefined && (
                        <div className="mt-1 flex items-center gap-2 text-xs">
                          <span className="text-slate-500">Current Stock: <strong>{insight.action_payload.currentStock}</strong></span>
                          <span className="text-slate-400">&rarr;</span>
                          <span className="text-emerald-600 dark:text-emerald-400 font-bold">
                            New Total: {Number(insight.action_payload.currentStock) + Number(adjustmentQty)} units
                          </span>
                        </div>
                      )}
                    </div>
                  ...
                  <div>
                    ...
                    <span className="text-[11px] text-slate-400 mt-1 block">
                      This quantity will be added (+{adjustmentQty}) to existing stock upon approval.
                    </span>
                  </div>
                </div>
              )}
