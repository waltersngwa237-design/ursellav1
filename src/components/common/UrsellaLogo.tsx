 ...
 * UrsellaSymbolMark
 * 
 * Abstract geometric octopus brand mark:
 * - Mantle dome (Intelligence Hub)
 * - Symmetrical 6 tentacles (Sales, Inventory, Finance, Customers, Analytics, Operations)
 * - Apex Core Diamond
 * - Micro focal node
 */
export const UrsellaSymbolMark: React.FC<{
  ...
  const isAI = theme === 'ai' || theme === 'purple';

  const rawId = React.useId();
  const uid = rawId.replace(/[^a-zA-Z0-9_-]/g, '_');

  const bodyGradId = `ursella_body_${uid}`;
  const coreGradId = `ursella_core_${uid}`;

  let bodyGradStart = '#34D399';
  let bodyGradMid = '#10B981';
  let bodyGradEnd = '#059669';
  let coreGradStart = '#A7F3D0';
  let coreGradEnd = '#34D399';
  let focalFill = '#FFFFFF';
    ...
    >
      <defs>
        <linearGradient id={bodyGradId} x1="0%" y1="0%" x2="100%" y2="100%">
          ...
          <stop offset="50%" stopColor={bodyGradMid} />
          <stop offset="100%" stopColor={bodyGradEnd} />
        </linearGradient>

        <linearGradient id={coreGradId} x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor={coreGradStart} />
          <stop offset="100%" stopColor={coreGradEnd} />
        </linearGradient>
      </defs>
      ...
      <g>
        ...
        {/* 6. Micro-Focal Point */}
        <circle cx="16" cy="9.9" r="0.75" fill={focalFill} opacity="0.95" />
      </g>
    </svg>
  );
};
  ...
  if (activeVariant === 'badge' || activeVariant === 'ai-badge') {
    ...
    const badgeBg =
      theme === 'mono-black'
        ...
        : 'bg-zinc-900/90 border border-emerald-500/25 shadow-md shadow-emerald-950/40';
  ...
  if (activeVariant === 'compact') {
    return (
      <div
        ...
        onClick={onClick}
      >
        <div className="p-2.5 rounded-2xl bg-zinc-900/90 border border-emerald-500/25 shadow-lg shadow-emerald-950/30 mb-2">
